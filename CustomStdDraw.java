 ...
 *  @author Kevin Wayne
 */
public final class CustomStdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static CustomStdDraw std = new CustomStdDraw();
    ...
    public static void safelySetCanvasSize(int canvasWidth, int canvasHeight) { // Doesn't work.
        if (canvasWidth <= 0) throw new IllegalArgumentException("width must be positive");
        if (canvasHeight <= 0) throw new IllegalArgumentException("height must be positive");
        width = canvasWidth;
        height = canvasHeight;
    ...
    private static void init() {
        ...
        frame.setTitle("Decibel Meter - StdDraw");
    ...
    private static Image getImage(String filename) {
        ...
        if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
            URL url = CustomStdDraw.class.getResource(filename);
            if (url != null)
                icon = new ImageIcon(url);
        }
    ...
    public void actionPerformed(ActionEvent e) {
        FileDialog chooser = new FileDialog(CustomStdDraw.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        String filename = chooser.getFile();
        if (filename != null) {
            CustomStdDraw.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = CustomStdDraw.userX(e.getX());
            mouseY = CustomStdDraw.userY(e.getY());
            isMousePressed = true;
        }
    }
    ...
    public static void main(String[] args) {
        CustomStdDraw.square(0.2, 0.8, 0.1);
        CustomStdDraw.filledSquare(0.8, 0.8, 0.2);
        CustomStdDraw.circle(0.8, 0.2, 0.2);

        CustomStdDraw.setPenColor(CustomStdDraw.BOOK_RED);
        CustomStdDraw.setPenRadius(0.02);
        CustomStdDraw.arc(0.8, 0.2, 0.1, 200, 45);

        // draw a blue diamond
        CustomStdDraw.setPenRadius();
        CustomStdDraw.setPenColor(CustomStdDraw.BOOK_BLUE);
        double[] x = { 0.1, 0.2, 0.3, 0.2 };
        double[] y = { 0.2, 0.3, 0.2, 0.1 };
        CustomStdDraw.filledPolygon(x, y);

        // text
        CustomStdDraw.setPenColor(CustomStdDraw.BLACK);
        CustomStdDraw.text(0.2, 0.5, "black text");
        CustomStdDraw.setPenColor(CustomStdDraw.WHITE);
        CustomStdDraw.text(0.8, 0.8, "white text");
    }
}
